...
WHERE pmc."productId" = p.id AND pmc."modifierCategoryId" = mc.id AND p."restaurantId" = r.id
  AND r.name = 'WokBox' AND p.name = 'Noodle Bar';
...
FROM "modifier_categories" mc JOIN "restaurants" r ON r.id = mc."restaurantId"
...
FROM "modifiers" m JOIN "modifier_categories" normal ON normal.id = m."categoryId" AND normal.name = 'Proteina'
...
FROM "products" p JOIN "restaurants" r ON r.id = p."restaurantId"
