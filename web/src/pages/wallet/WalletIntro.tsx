...
/**
 * Telón de entrada a QuickTap Wallet.
 *
 ...
 * ejemplo .wallet-panel, que se anima al entrar). Un transform en un ancestro hace que este
 ...
 */
export function WalletIntro({ saliendo }: { saliendo: boolean }) {
  return (
    <div
      ...
      className={`fixed inset-0 z-50 flex flex-col items-center justify-center bg-[#04070d] ${
        saliendo ? 'wallet-intro--saliendo pointer-events-none' : ''
      }`}
    >
      <img src={WALLET_WORDMARK_URL} alt={WALLET_NAME} className="wallet-intro-logo h-12 w-auto" />
    </div>
  );
}
