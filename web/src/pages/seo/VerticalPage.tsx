...
export default function VerticalPage() {
  ...
  return (
    <SeoPageLayout>
      ...
      <p className="text-xs font-bold uppercase tracking-widest text-brand-500">QuickTap para tu negocio</p>
      ...
      <section className="mt-10 grid sm:grid-cols-3 gap-5">
        {page.points.map((pt) => (
          <Link
            key={pt.title}
            to={pt.to}
            className="group rounded-2xl border border-brand-950/[0.08] p-5 hover:border-brand-500/40 transition-colors"
          >
            <h2 className="text-[15px] font-semibold mb-1.5 group-hover:text-brand-500 transition-colors">{pt.title}</h2>
