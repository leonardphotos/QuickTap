...
if not exist "%SHORTCUT%" (
    powershell -NoProfile -Command "$s=(New-Object -COM WScript.Shell).CreateShortcut('%SHORTCUT%'); $s.TargetPath='%~f0'; $s.WorkingDirectory='%~dp0'; $s.IconLocation='shell32.dll,138'; $s.Description='Estacion de Impresion QuickTap'; $s.Save()"
)
