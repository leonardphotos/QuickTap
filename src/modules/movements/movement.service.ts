    ...
    return prisma.$transaction(async (tx) => {
      const movement = await tx.movement.create({
        data: {
          ...
          shopEventProductId: input.shopEventProductId ?? undefined,
  ...
  async list(restaurantId: string, query: MovementQuery) {
    ...
    return {
      ...
      movements: movements.map((m) => ({
        ...
        category: m.category,
        // Evento al que se imputó el gasto: el formulario lo necesita para precargarlo al editar.
        shopEventProductId: m.shopEventProductId,
    ...
    return prisma.$transaction(async (tx) => {
      ...
      return tx.movement.update({
        ...
        data: {
          ...
          paymentMethod: input.paymentMethod,
          category: input.category,
          shopEventProductId: input.shopEventProductId,
          supplierId: input.supplierId,
