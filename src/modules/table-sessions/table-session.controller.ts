...
import { closeTableSessionSchema, moveTableSessionSchema } from './table-session.dto';
import { tableSessionService } from './table-session.service';

export const tableSessionController = {
  close: asyncHandler(async (req: Request, res: Response) => {
    const input = closeTableSessionSchema.parse(req.body);
    res.json({ data: await tableSessionService.close(req.restaurantId!, req.params.id, input.paymentMethod) });
  }),
