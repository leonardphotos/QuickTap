  ...
  async createProduct(restaurantId: string, input: CreateShopProductInput) {
    const product = await prisma.shopProduct.create({
      data: {
        ...
        eventImages: input.eventImages ?? undefined,
        eventTerms: input.eventTerms ?? null,
        eventFinancingEnabled: input.eventFinancingEnabled ?? false,
        ...
        // El resto del catálogo sigue naciendo oculto (hay insumos internos que no se venden).
        isPublished: input.isPublished ?? input.isEvent ?? false,
        price: input.price,
        cost: input.cost,
